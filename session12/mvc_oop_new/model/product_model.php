...
class ProductModel extends ConnectDB{
	...
	public function getListProduct($page){
			...
			$sql = "SELECT products.id, products.name, products.id, categories.id as category_id, categories.name as category_name  FROM products INNER JOIN categories ON products.category_id = categories.id  WHERE products.category_id = 2 LIMIT ".$start.",".self::PAGING;
